...
const ActiveOrderButton:FC<{activeIndex : number,clickHandler : (index : number) => void}> = ({activeIndex,clickHandler}) => {
    const index = 0
    return(
        ...
        >
            <button
                className="text-base text-[#AEAEAE] duration-200 group-hover:text-[#46B1F0] text-left md:order-1 md:text-right">Поточні
                замовлення</button>
           <TabButtonArrow/>
        </li>
    )
